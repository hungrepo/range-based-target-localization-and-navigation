...
%    r_hat = p - q_hat;
    ...
    elseif (filter == 2)
        x_hat = [r_hat; r_hat'*v_hat, norm(r_hat)^2, norm(v_hat)^2, v_hat];
        P_hat = diag([250,150, 0.1, 0.1,0.1, 0.5,0.5]);                                       % Initial estimate of covariance
    elseif (filter == 3)
    ...
    elseif (filter == 7)
        x_hat = [q_hat; q_hat'*v_hat; norm(q_hat)^2; norm(v_hat)^2; v_hat];
        P_hat = diag([250,150, 10, 1000, 1, 0.5,0.5]);                                       % Initial estimate of covariance
...
% Tracker trajectory and input
    p_meas(:,end+1) = p(:,end) +  0*randn(2,1);
...
% Simulate/update position and velocity of target 
   % Note: if you want to change the desired target trajectory to track, change q and v
    q(:,end+1) = [1*20*sin(0.01*t)+10;  0.3*t ];                             % target position
    v(:,end+1) = [1*0.2*cos(0.01*t); 0.3];
...
% Simulate range to target     
    r(:,end+1) = p(:,end) - q(:,end);
    if t==0
        range(end+1) = norm([r(:,end);Target.Depth]) + 0*randn;
    else
        range(end+1) = norm([r(:,end);Target.Depth]) + 0*randn;
    end
%   x(:,end+1) = [r; v(:,end)]; 
...
% Estimate the target state
    if (filter ==1)
        [x_hat(:,end+1),P_hat] = STST_EKF_PosVel(range(end),p_meas(:,end),x_hat(:,end),P_hat,Ts,Target.Depth,t);
    ...
    elseif(filter == 7)
     %   lambda(:,end+1) = p(:,end)-p(:,1);
        y(end+1) = range(end)^2 - norm(p_meas(:,end))^2; 
        [x_hat(:,end+1),P_hat] = STST_KF_PosVel_LTV5_2D(y(end),x_hat(:,end),P_hat,Ts,Target.Depth,t, p_meas(:,end));
         q_hat(:,end+1) = x_hat(1:2,end) ;
         v_hat(:,end+1) = x_hat(6:7,end);
         x(:,end+1) = [q(:,end); q(:,end)'*v(:,end); norm(q(:,end))^2; norm(v(:,end))^2; v(:,end) ];                
    end
    P_save = [P_save P_hat];
